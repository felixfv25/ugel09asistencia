...
const filtroVisitas = document.getElementById("filtro-visitas");
const ayudaFiltroVisitas = document.getElementById("ayuda-filtro-visitas");
...
function renderizarVisitasHoy() {
  const termino = normalizarTexto(filtroVisitas.value);
  const visitasFiltradas = termino
    ? visitasHoy.filter((r) => {
        const dni = normalizarTexto(r.dni);
        const nombre = normalizarTexto(`${r.nombres} ${r.apellidos}`);
  ...
  if (visitasHoy.length === 0) {
    ...
    ayudaFiltroVisitas.textContent = "Puedes buscar por nombre o DNI.";
    return;
  }

  if (visitasFiltradas.length === 0) {
    cuerpoTabla.innerHTML = `<tr><td colspan="7" class="celda-vacia">No se encontró ninguna visita con “${escaparHtml(filtroVisitas.value)}”.</td></tr>`;
    ayudaFiltroVisitas.textContent = `${visitasHoy.length} visita(s) registradas hoy.`;
    return;
  }

  ayudaFiltroVisitas.textContent = termino
    ? `Mostrando ${visitasFiltradas.length} de ${visitasHoy.length} visita(s).`
    : `${visitasHoy.length} visita(s) registradas hoy.`;

  cuerpoTabla.innerHTML = visitasFiltradas
    .map((r) => {
...
function cargarVisitasHoy() {
  fetch(`${API_URL}/registros/hoy`, {
    ...
    .then((res) => res.json())
    .then((data) => {
      visitasHoy = data.registros || [];
      renderizarVisitasHoy();
    })
    .catch(() => {
      visitasHoy = [];
      cuerpoTabla.innerHTML = `<tr><td colspan="7" class="celda-vacia">No se pudo cargar la lista.</td></tr>`;
      ayudaFiltroVisitas.textContent = "No se pudo cargar la lista de visitas.";
    });
}
...
filtroVisitas.addEventListener("input", renderizarVisitasHoy);
